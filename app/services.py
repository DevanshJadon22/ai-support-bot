...
def get_llm_response(session_id, user_query):
    ...
    for i, msg in enumerate(history_messages):
        ...
        # Prepend the system prompt to the very first user message in the history
        if i == 0 and role == "user":
            content = system_prompt + "\n\nUser question: " + msg.content
            messages_for_gemini.append({"role": role, "parts": [content]})
        else:
            messages_for_gemini.append({"role": role, "parts": [msg.content]})
    ...
    try:
        ...
        # Pass the full constructed history to the model
    ...
    if "ESCALATE" in assistant_response:
        summary = summarize_conversation(history_messages) # Use the original message objects
        session = ChatSession.query.get(session_id)
        session.summary = summary
        db.session.commit()
        response_data = {
